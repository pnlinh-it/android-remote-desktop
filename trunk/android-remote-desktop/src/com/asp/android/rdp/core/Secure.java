 ...
 * Date: $Date: 2008-01-25 16:56:38 +0530 (Fri, 25 Jan 2008) $
 *
...
import com.asp.android.rdp.crypto.BlockMessageDigest;
import com.asp.android.rdp.crypto.CryptoException;
import com.asp.android.rdp.crypto.MD5;
import com.asp.android.rdp.crypto.RC4;
import com.asp.android.rdp.crypto.SHA1;
...
public class Secure {
	 ...
	 */
	public Secure(RDPConnection controller, VChannels channels) {
		this.rdpConnection=controller;
		this.channels = channels;
		McsLayer = new MCS(controller,channels);
		controller.setMcs(McsLayer);
		rc4_dec = new RC4();
		rc4_enc = new RC4();
		rc4_update = new RC4();
		sha1 = new SHA1();
	...
	public void connect(InetAddress host, int port)
		...
		if (rdpConnection.getOptions().getHostname() == "") {
			InetAddress localhost = InetAddress.getLocalHost();
			String name = localhost.getHostName();
			StringTokenizer tok = new StringTokenizer(name, ".");
			rdpConnection.getOptions().setHostname(tok.nextToken());
			rdpConnection.getOptions().getHostname().trim();
		}
	...
	public void connect(InetAddress host) throws IOException,
		...
		this.connect(host, rdpConnection.getOptions().getPort());
	}
	...
	public RdpPacket_Localised sendMcsData() {
		...
		int hostlen = 2 * (rdpConnection.getOptions().getHostname() == null ? 0 : rdpConnection.getOptions().getHostname()
				.length());
		...
		int length = 158;
		if (rdpConnection.getOptions().isUse_rdp5())
			length += 76 + 12 + 4;

		if (rdpConnection.getOptions().isUse_rdp5() && (channels.num_channels() > 0))
		...
		buffer.setLittleEndian16(rdpConnection.getOptions().isUse_rdp5() ? 212 : 136); // length
		buffer.setLittleEndian16(rdpConnection.getOptions().isUse_rdp5() ? 4 : 1);
		buffer.setLittleEndian16(8);
		buffer.setLittleEndian16(rdpConnection.getOptions().getWidth());
		buffer.setLittleEndian16(rdpConnection.getOptions().getHeight());
		buffer.setLittleEndian16(0xca01);
		buffer.setLittleEndian16(0xaa03);
		buffer.setLittleEndian32(rdpConnection.getOptions().getKeylayout());
		buffer.setLittleEndian32(rdpConnection.getOptions().isUse_rdp5() ? 2600 : 419); // or 0ece
		// // client
		// build? we
		// are 2600
		// compatible
		...
		buffer.outUnicodeString(rdpConnection.getOptions().getHostname().toUpperCase(), hostlen);
		...
		buffer.setLittleEndian16(rdpConnection.getOptions().isUse_rdp5() ? 1 : 0);

		if (rdpConnection.getOptions().isUse_rdp5()) {
			buffer.setLittleEndian32(0); // out_uint32(s, 0);
			buffer.set8(rdpConnection.getOptions().getServer_bpp()); // out_uint8(s, g_server_bpp);
			...
			buffer.setLittleEndian32(rdpConnection.getOptions().isConsole_session() ? 0xb : 0xd); // out_uint32_le(s,
			// g_console_session
			// ?
			// 0xb
			// :
		...
		buffer.setLittleEndian16(rdpConnection.getOptions().isUse_rdp5() ? 12 : 8); // length

		// if(Options.use_rdp5) buffer.setLittleEndian32(Options.encryption ?
		// 0x1b : 0); // 128-bit encryption supported
		// else
		buffer
				.setLittleEndian32(rdpConnection.getOptions().isEncryption() ? (rdpConnection.getOptions().isConsole_session() ? 0xb
						: 0x3)
						: 0);
	...
	private void processSrvInfo(RdpPacket_Localised mcs_data) {
		rdpConnection.getOptions().setServer_rdp_version(mcs_data.getLittleEndian16()); // in_uint16_le(s,
		// g_server_rdp_version);
		if (1 == rdpConnection.getOptions().getServer_rdp_version())
			rdpConnection.getOptions().setUse_rdp5(false);
	}
	...
	public void send(RdpPacket_Localised sec_data, int flags)
		...
		RDPLogger.d("Sending :Secure layer");
	...
	public RdpPacket_Localised receive() throws RdesktopException, IOException,
			CryptoException, OrderException, ConnectionException {
		int sec_flags = 0;
