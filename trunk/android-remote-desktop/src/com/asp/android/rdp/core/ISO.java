...
public abstract class ISO {
	...
	private RDPConnection controller;
	/**
	 * Construct ISO object, initialises hex dump
	 */
	public ISO(RDPConnection controller) {
	...
	protected void doSocketConnect(InetAddress host, int port)
			throws IOException {
		RDPLogger.d("Socket connection initialization"+host.getHostAddress()+"Port :"+port);
	...
	public void connect(InetAddress host, int port) throws IOException,
		...
		RDPLogger.d("ISO layer connection");
		int[] code = new int[1];
		doSocketConnect(host, port);
		RDPLogger.d("Connection Initialised");
		rdpsock.setTcpNoDelay(controller.getOptions().isLow_latency());
		// this.in = new InputStreamReader(rdpsock.getInputStream());
		
		// Initialize the input stream to read from the server.
	...
	private RdpPacket_Localised receiveMessage(int[] type) throws IOException,
		...
		next_packet: while (true) {
			...
			if ((version & 3) == 0) {
				...
				controller.getRdp().rdp5_process(s, (version & 0x80) != 0);
	...
	void send_connection_request() throws IOException {
		...
		String uname = controller.getOptions().getUsername();
		if (uname.length() > 9)
			uname = uname.substring(0, 9);
		int length = 11 + (controller.getOptions().getUsername().length() > 0 ? ("Cookie: mstshash="
				.length()
				+ uname.length() + 2) : 0) + 8;
		...
		buffer.setBigEndian16(0); // source reference should be a reasonable
									// address we use 0
		buffer.set8(0); // service class
		if (controller.getOptions().getUsername().length() > 0) {
			buffer
