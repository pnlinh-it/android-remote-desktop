...
public class Rdp {
	...
	public static final int INFO_REMOTECONSOLEAUDIO=0x2000;
	...
	private static final int PDUTYPE2_SUPPRESS_OUTPUT=35;
	...
	private static final int SYSPTR_DEFAULT = 0x00007F00;
	...
	private BackstoreManager surface = null;
	 ...
	 */
	void processGeneralCaps(RdpPacket_Localised data) {
		...
		if (pad2octetsB != 0)
			rdpConnection.getOptions().use_rdp5 = false;
	}
	 ...
	 *            Packet containing capability set data at current read position
	 */
	void processBitmapCaps(RdpPacket_Localised data) {
		 ...
		 */
		if (rdpConnection.getOptions().server_bpp != bpp) {

			rdpConnection.getOptions().server_bpp = bpp;
		}
		if (rdpConnection.getOptions().width != width
				|| rdpConnection.getOptions().height != height) {

			rdpConnection.getOptions().width = width;
			rdpConnection.getOptions().height = height;
	 ...
	 */
	public Rdp(RDPConnection rdpConnection, VChannels channels) {
		this.rdpConnection = rdpConnection;
		this.SecureLayer = new Secure(rdpConnection, channels);
		rdpConnection.setSecure(SecureLayer);
		this.orders = new Orders(rdpConnection);
	 ...
	 * @throws ConnectionException
	 */
	public boolean connect(String username, InetAddress server, int flags,
		...
		// Handle an unresolvable hostname
			...
			RDPLogger.d("EXCEPTION :Could not resolve host name: " + server);
			throw new ConnectionException("Could not resolve host name: "
					+ server);
		}
		// Handle a refused connection
		catch (ConnectException e) {
			RDPLogger.d("Connection refused when trying to connect to "
					+ server + " on port " + rdpConnection.getOptions().port);
			throw new ConnectionException(
					"Connection refused when trying to connect to " + server
							+ " on port " + rdpConnection.getOptions().port);
		}
	...
	public void mainLoop(boolean[] deactivated, int[] ext_disc_reason)
			...
			CryptoException, ConnectionException {
	...
	private void sendLogonInfo(int flags, String domain, String username,
		...
		if (!rdpConnection.getOptions().use_rdp5
				|| 1 == rdpConnection.getOptions().server_rdp_version) {
			...
			data
					.setLittleEndian32(rdpConnection.getOptions().rdp5_performanceflags); // out_uint32_le(s,
			// 0x0f);
			data.setLittleEndian32(0); // out_uint32(s, 0);
		}
	...
	private void processDemandActive(RdpPacket_Localised data)
			throws RdesktopException, IOException, CryptoException,
			OrderException, ConnectionException {
		int type[] = new int[1];
	...
	private boolean processData(RdpPacket_Localised data, int[] ext_disc_reason)
		...
		case RDP_DATA_PDU_DISCONNECT:
			...
			return false; // Windows vista issue fix.
	...
	private void processUpdate(RdpPacket_Localised data) throws OrderException,
		...
		case (Rdp.RDP_UPDATE_BITMAP):
			System.out.println("Process BITMAP updates");
	...
	private void sendConfirmActive() throws RdesktopException, IOException,
		...
		int sec_flags = rdpConnection.getOptions().encryption ? (RDP5_FLAG | Secure.SEC_ENCRYPT)
				: RDP5_FLAG;
		...
		data
				.setLittleEndian16(rdpConnection.getMcs().getUserID() /*
																	 * McsUserID(
																	 * )
																	 */+ 1001);
		...
		if (rdpConnection.getOptions().use_rdp5
				&& rdpConnection.getOptions().persistent_bitmap_caching) {
		...
		rdpConnection.getSecure().send(data, sec_flags);
	}
	...
	private void sendGeneralCaps(RdpPacket_Localised data) {
		...
		data.setLittleEndian16(rdpConnection.getOptions().use_rdp5 ? 0x40d : 0);
		// data.setLittleEndian16(rdpConnection.getOptions().use_rdp5 ? 0x1d04 :
		// 0); // this seems
		/*
		...
		data.set8(0x01); /* Refresh rectangle */
		data.set8(0x01); /* Suppress output */
	}
	...
	private void sendBitmapCaps(RdpPacket_Localised data) {
		...
		data.setLittleEndian16(rdpConnection.getOptions().server_bpp); /*
																		 * Preferred
																		 * BPP
																		 */
		...
		data.setLittleEndian16(rdpConnection.getOptions().width); /* Desktop width */
		data.setLittleEndian16(rdpConnection.getOptions().height); /*
																	 * Desktop
																	 * height
																	 */
		data.setLittleEndian16(0); /* Pad */
		...
		data
				.setLittleEndian16(rdpConnection.getOptions().bitmap_compression ? 1
						: 0); /*
							 * Support compression
							 */
	...
	private void sendOrderCaps(RdpPacket_Localised data) {
		...
		order_caps[3] = (byte) (rdpConnection.getOptions().bitmap_caching ? 1
				: 0); /* memblt */
		...
		order_caps[20] = (byte) (rdpConnection.getOptions().polygon_ellipse_orders ? 1
				: 0); /* polygon */
		order_caps[21] = (byte) (rdpConnection.getOptions().polygon_ellipse_orders ? 1
				: 0); /* polygon2 */
		order_caps[22] = 1; /* polyline */
		order_caps[25] = (byte) (rdpConnection.getOptions().polygon_ellipse_orders ? 1
				: 0); /* ellipse */
		order_caps[26] = (byte) (rdpConnection.getOptions().polygon_ellipse_orders ? 1
				: 0); /* ellipse2 */
	...
	private void sendBitmapcacheCaps(RdpPacket_Localised data) {
		...
		data.setLittleEndian16(0x00C8); /* entries */
	...
	private void sendBitmapcache2Caps(RdpPacket_Localised data) {
		...
		data
				.setLittleEndian16(rdpConnection.getOptions().persistent_bitmap_caching ? 2
						: 0); /* version */
	...
	private void sendPointerCaps(RdpPacket_Localised data) {
		...
		data.setLittleEndian16(20); /* color pointer Cache size */
		data.setLittleEndian16(20); /* pointer Cache size */
	}
	...
	public void sendInput(int time, int message_type, int device_flags,
		...
		try {
			...
			RDPLogger.d("Exception in sending");
		}
	...
	private void processPointer(RdpPacket_Localised data)
		...
		case (Rdp.TS_PTRMSGTYPE_POSITION):
			RDPLogger.d("Pointer move");
		...
		case (Rdp.TS_PTRMSGTYPE_COLOR):
			RDPLogger.d("Pointer color");
			process_colour_pointer_pdu(data);
			break;

		case (Rdp.TS_PTRMSGTYPE_CACHED):
			RDPLogger.d("Pointer cached");
			process_cached_pointer_pdu(data);
			break;

		case TS_PTRMSGTYPE_SYSTEM:
			RDPLogger.d("Pointer system");
	...
	protected void processBitmapUpdates(RdpPacket_Localised data)
		...
		maxX = maxY = 0;
		minX = rdpConnection.getOptions().width;// surface.getWidth();
		minY = rdpConnection.getOptions().height;// surface.getHeight();
		...
		for (int i = 0; i < n_updates; i++) {
			...
			if (rdpConnection.getOptions().server_bpp != bitsperpixel) {

				rdpConnection.getOptions().set_bpp(bitsperpixel);
			}
			...
			if (compression == 0) {
				...
				surface.displayImage(Bitmap.convertImage(pixel, Bpp,
						rdpConnection.getOptions().server_bpp), width, height,
						left, top, cx, cy);
				continue;
			}
			...
			if (Bpp == 1) {
				...
				if (pixel != null)
					surface.displayImage(Bitmap.convertImage(pixel, Bpp,
							rdpConnection.getOptions().server_bpp), width,
							height, left, top, cx, cy);
			} else {

				if (rdpConnection.getOptions().bitmap_decompression_store == rdpConnection
						.getOptions().INTEGER_BITMAP_DECOMPRESSION) {
					//					

					int[] pixeli = Bitmap.decompressInt(width, height, size,
							data, Bpp, rdpConnection.getOptions().server_bpp);

					surface.displayImage(pixeli, width, height, left, top, cx,
							cy);
	...
	public void registerBackstoreManager(BackstoreManager backstoreManager) {
		this.surface = backstoreManager;
		orders.registerBackstoreManager(backstoreManager);
	}
	...
	protected void process_colour_pointer_pdu(RdpPacket_Localised data)
			throws RdesktopException {
		RDPLogger.d("Pointer operation color pointer");
	...
	protected void process_cached_pointer_pdu(RdpPacket_Localised data)
			throws RdesktopException {
		RDPLogger.d("Pointer operation cached pointer");
		int cache_idx = data.getLittleEndian16();
		// logger.info("Setting cursor "+cache_idx);
		// surface.setCursor(cache.getCursor(cache_idx));
	}
	
	private static final int SUPPRESS_DISPLAY_UPDATES=0x00;
	public void pause(){
		RdpPacket_Localised data;
		try {
			data=this.initData(4);
		} catch (RdesktopException e) {
			return;
		}
		data.set8(SUPPRESS_DISPLAY_UPDATES);
		
		data.set8(0); // Pad3Octets
		data.setLittleEndian16(0);
		data.markEnd();
		
		try {
			this.sendData(data, PDUTYPE2_SUPPRESS_OUTPUT);
		} catch (RdesktopException e) {
		} catch (IOException e) {
		} catch (CryptoException e) {
	...
	private static final int ALLOW_DISPLAY_UPDATES=0x01;
	public void resume(){
		RdpPacket_Localised data;
		try {
		...
		data.set8(ALLOW_DISPLAY_UPDATES);
		
		data.set8(0); // Pad3Octets
		data.setLittleEndian16(0);
	    
		data.setLittleEndian16(0); // left
		data.setLittleEndian16(0); // top
		data.setLittleEndian16(rdpConnection.getOptions().width); // right
		data.setLittleEndian16(rdpConnection.getOptions().height);//bottom
