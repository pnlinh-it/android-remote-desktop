...
public abstract class VChannel {
	...
	public VChannel(RDPConnection controller) {
	...
	public RdpPacket_Localised init(int length) throws RdesktopException {
		...
		s = controller.getSecure().init(controller.getOptions().encryption ? Secure.SEC_ENCRYPT : 0,
				length + 8);
	...
	public void send_packet(RdpPacket_Localised data) throws RdesktopException,
		...
		while (data_offset < length) {
			...
			RdpPacket_Localised s = controller.getSecure().init(
			...
			if (controller.getSecure() != null)
				controller.getSecure().send_to_channel(s,
