...
import com.asp.android.rdp.core.RdpPacket_Localised;
...
public class Bitmap {
	...
	public static int convertTo24(int colour,int server_bpp) {
		if (server_bpp == 15)
	 ...
	 * @return
	 */
	static int cvalx(byte[] data, int offset, int Bpp,int server_bpp) {
		int rv = 0;
		if (server_bpp == 15) {
	 ...
	 */
	public static int[] convertImage(byte[] bitmap, int Bpp,int server_bpp) {
		...
		for (int i = 0; i < out.length; i++) {
			if (Bpp == 1)
				...
				out[i] = ((bitmap[i * Bpp + 2] & 0xFF) << 16)
						...
						| (bitmap[i * Bpp] & 0xFF);
			out[i] = Bitmap.convertTo24(out[i],server_bpp);
		}
		return out;
	}
	 ...
	 */
	public Bitmap(byte[] data, int width, int height, int x, int y, int Bpp,int server_bpp) {
		this.highdata = Bitmap.convertImage(data, Bpp,server_bpp);
		this.width = width;
		this.height = height;
		this.x = x;
		this.y = y;
	...
	public static int[] decompressInt(int width, int height, int size,
			RdpPacket_Localised data, int Bpp,int server_bpp) throws RdesktopException {
		...
		while (input < end) {
			...
			case 0: /* Fill */
				...
				break;
			case 8: /* Bicolor */
				color1 = cvalx(compressed_pixel, input, Bpp,server_bpp);
				// (compressed_pixel[input++]&0x000000ff);
				input += Bpp;
			case 3: /* Color */
				color2 = cvalx(compressed_pixel, input, Bpp,server_bpp);
			...
			case 7: /* SetMix/FillOrMix */
				...
				mix = cvalx(compressed_pixel, input, Bpp,server_bpp);
				input += Bpp;
				opcode -= 5;
				break;
			case 9: /* FillOrMix_1 */
			...
			while (count > 0) {
				...
				case 4: /* Copy */
					...
					while ((count > 0) && (x < width)) {
						// setli(pixel, line, x, cvalx(compressed_pixel,
						// input,Bpp), Bpp);
						pixel[line + x] = cvalx(compressed_pixel, input, Bpp,server_bpp);
						input += Bpp;
						// pixel[line+x] = compressed_pixel[input++];
						count--;
						x++;
