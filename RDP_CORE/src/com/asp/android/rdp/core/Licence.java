...
public class Licence {
	...
	public byte[] generate_hwid() throws UnsupportedEncodingException {
		...
		byte[] name = secure.getRdpConnection().getOptions().getUsername().getBytes("US-ASCII");
	...
	public void process_demand(RdpPacket_Localised data)
		...
		byte[] host = secure.getRdpConnection().getOptions().getUsername().getBytes("US-ASCII");
		byte[] user = secure.getRdpConnection().getOptions().getUsername().getBytes("US-ASCII");
		...
		if (!secure.getRdpConnection().getOptions().isBuilt_in_licence() && secure.getRdpConnection().getOptions().isLoad_licence()) {
	...
	public void process_issue(RdpPacket_Localised data) throws CryptoException {
		...
		if (secure.getRdpConnection().getOptions().isSave_licence())
	...
	public void send_request(byte[] client_random, byte[] rsa_data,
		...
		if (secure.getRdpConnection().getOptions().isBuilt_in_licence() && (!secure.getRdpConnection().getOptions().isLoad_licence())
				&& (!secure.getRdpConnection().getOptions().isSave_licence())) {
	...
	byte[] load_licence() {
		...
		return (new LicenceStore_Localised()).load_licence(secure.getRdpConnection().getOptions().getHostname());
	}
	...
	void save_licence(RdpPacket_Localised data, int length) {
		...
		new LicenceStore_Localised().save_licence(secure.getRdpConnection().getOptions().getHostname(),databytes);
